...
fn main() {
    let mut kol: usize = 0;
    while PathBuf::from(format!("{}.dat", num(kol))).exists() {
        kol += 1;
    }
    ...
    for i in 0..kol {
        let mut txt_dat = BufReader::new(File::open(format!("{}.dat", num(i))).unwrap());
        txt_dat.read_exact(&mut twobvec).unwrap();
        buf[i*CHUNK_SZ] = twobvec[0];
        buf[i*CHUNK_SZ+1] = twobvec[1];
        drop(txt_dat);

        let mut txt_txt = BufReader::new(File::open(format!("{}.txt", num(i))).unwrap());
        for j in 0..8 {
